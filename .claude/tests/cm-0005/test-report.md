...
No unit test framework is installed or configured. `client/package.json` and `server/package.json` both lack a `test` script. No `vitest.config.*` or `jest.config.*` files were found.
