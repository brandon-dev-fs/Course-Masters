...
export default function UnitForm({ initial, nextOrder = 1, onSubmit, onCancel }: UnitFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      <div>
        <Input id="title" label="Title" value={title} onChange={e => setTitle(e.target.value)} placeholder="e.g. Chapter 1" maxLength={30} autoFocus />
      ...
      <div className="flex justify-end gap-3 pt-2">
        ...
        <Button type="submit" disabled={!(title.trim().length > 0 && description.trim().length > 0) || submitting}>{submitting ? 'Saving...' : initial?.id ? 'Save Changes' : 'Add Unit'}</Button>
      </div>
    </form>
  );
}
